...
run() ->
  ...
  Request = "3f\x90\x01\x00\x00\x01\x00\x00\x00\x00\x00\x00\x06\x67\x6f\x6f\x67\x6c\x65\x03\x63\x6f\x6d\x00\x00\x01\x00\x01",
...
build_dns_request(DomainName) ->
  TransactionId = 63,
  Flags = <<0100>>,
  NumberOfQuestions = <<0001>>,
  AnswerResourceRecords = <<0000>>,
  AuthorityResourceRecords = <<0000>>,
  AdditionalResourceRecords = <<0000>>,
  Queries = "",

  QueryList = string:tokens(DomainName, "."),

  lists:map(fun(X) -> [length(X), X] end, QueryList),

  QueryListTransformed = lists:map(fun(X) ->
    [binary_encode_unsigned(length(X)), list_to_binary(lists:flatmap(fun erlang:integer_to_list/1, X))] end, QueryList),
  QueryBinary = lists:io_list_to_binary(lists:flatten(QueryListTransformed)),
  QueryEnd = <<00000000>>,
  RecordType = <<0001>>,
  Class = <<0001>>,

  [TransactionId, Flags, NumberOfQuestions, AnswerResourceRecords, AuthorityResourceRecords, AdditionalResourceRecords],
  {ok}.
